...
app.config(function(uiGmapGoogleMapApiProvider,$routeProvider, $locationProvider){
  ...
  when('/stores',{templateUrl: '../Views/stores.html',controller: 'mainController'}).
...
function createValidation(username,password,conPassword,name,surname,sSurname,id,date,email){
  ...
  !isBlank(sSurname) && !isBlank(id)  && !isBlank(date) &&!isBlank(email))
    {return true;}
  else
  {
    return false;}
...
app.controller("storeController",["uiGmapGoogleMapApi","$scope","$http","$location","$routeParams","userService",'directionService',
function(uiGmapGoogleMapApi,$scope,$http,$location,$routeParams,userService,directionService){
  $scope.map = {center: {latitude: 10.9517300, longitude: -85.1361000 }, zoom: 14 };
  $scope.options = {scrollwheel: false};
  $scope.showStore = function(latitud,longitude){
  console.log(latitud);
  $scope.map = {center: {latitude: latitud, longitude: longitude }, zoom: 14 };
  $scope.options = {scrollwheel: false};
}
